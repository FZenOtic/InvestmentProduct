    ...
    <div v-else-if="isGameOver" class="summary-content fade-in">
        <div class="summary-card">
            ...
            <div class="leaderboard-section fade-in">
                <h3>🏆 Top 10 Investors (Group 3)</h3>
...
const fetchLeaderboard = () => {
    const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3000';
    axios.get(`${API_URL}/api/leaderboard/Group3`)
        .then(res => {
...
@media screen and (max-width: 768px) {
    ...
    /* AI Advice */
    .ai-advice-section { margin-bottom: 25px; padding: 0 10px; }
    .ai-header { font-size: clamp(1.3rem, 5vw, 1.6rem); text-align: center; margin-bottom: 15px; }
    .ai-box { font-size: clamp(1.1rem, 4vw, 1.4rem); padding: 20px 15px; line-height: 1.7; border-radius: 8px; word-wrap: break-word; }
